...
def init_params(n_H0, n_W0, n_C0, n_y):
	...
	s_p2 = 2
	p_p2 = 0
	n_H2 = floor((n_H2 - f_p2) / s_p2) + 1 #13
	n_W2 = floor((n_W2 - f_p2) / s_p2) + 1 #13
	...
	#CONV 4 ("SAME")
	n_C4 = 256	#previously 384
	f4 = 3	#filter_size
	s4 = 1	#stride
	p4 = (f4 - 1) / 2	#padding for SAME CONV
	W4 = tf.get_variable(dtype = tf.float32,
		shape = [f4, f4, 96, n_C4],	#n_C3 changed to 96 because Z4 is now directly taking input from P2
		initializer = tf.contrib.layers.xavier_initializer(),
		name = 'W4')
	...
	#POOL 6
	f_p6 = 2	#previously 3
	s_p6 = 2	#previously 2, then chaned to 1, then back to 2 (currently, value is 2)
	p_p6 = 0
	n_H6 = floor((n_H4 - f_p6) / s_p6) + 1
	n_W6 = floor((n_W4 - f_p6) / s_p6) + 1
...
def forward_prop(X_train, params, architecture_hparams):
	...
	f_p2, s_p2, p_p2 = architecture_hparams["POOL2"]
	...
	P2 = tf.nn.max_pool(value = A2, ksize = [1, f_p2, f_p2, 1], strides = [1, s_p2, s_p2, 1], padding = "VALID", data_format = "NHWC", name = "P2")
	...
	Z4 = tf.nn.conv2d(input = P2, filter = W4, strides = [1, s4, s4, 1], padding = "SAME", data_format = "NHWC", name = "Z4")
	...
	P6 = tf.nn.max_pool(value = A4, ksize = [1, f_p6, f_p6, 1], strides = [1, s_p6, s_p6, 1], padding = "VALID", data_format = "NHWC", name = "P6")
	
	P6_flat = tf.contrib.layers.flatten(inputs = P6)

	Z7 = tf.contrib.layers.fully_connected(inputs = P6_flat, num_outputs = 4096, normalizer_fn = tf.contrib.layers.batch_norm)
	A7 = tf.nn.relu(Z7, name = "A7")	

	Z8 = tf.contrib.layers.fully_connected(inputs = A7, num_outputs = 10, normalizer_fn = tf.contrib.layers.batch_norm)
